...
| リソース           | 概要                                                       |
|--------------------|------------------------------------------------------------|
| VPC                | CIDR `10.0.0.0/24` のカスタムVPCを作成                      |
| Subnet             | パブリックサブネットを2つ作成（AZ: a, b）でマルチAZ構成を意識   |
...
- Key Pair：
EC2インスタンスへのSSH接続用に鍵認証を使用しました。本番環境ではキーペア管理のリスクを避けるためSSM Session Manager への切り替えを想定しています。
